...
lastReviewedCommit: d5b8591d2276e4f2ea26e21064c468657d19e8bc
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
2. whether `uv run python scripts/schema_lock.py check` ran when schema assets or workflows changed
3. which manual CLI probes ran, if any, including the data format used for validation probes
4. whether downstream `tidas-sdk` follow-up is required
5. whether any export proof is deferred because it depends on live DB or storage state
