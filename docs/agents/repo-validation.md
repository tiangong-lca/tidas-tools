...
| `tidas-assets` or executable assets | asset-lock check, Rust tests, Python schema lock, full pytest | regenerate the asset lock only after intentional review; compare asset fingerprint twice | The Rust asset lock covers more executable inputs than the legacy paired-schema lock; both gates remain during migration. |
